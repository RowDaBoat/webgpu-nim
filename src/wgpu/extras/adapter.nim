...
proc features *(adapter :wgpu.Adapter) :seq[wgpu.FeatureName]=
  ...
  result = newSeqWith(data.featureCount.int, wgpu.FeatureName.Force32)
...
proc info *(adapter :wgpu.Adapter) :extras.AdapterInfo=
  ...
  if status != wgpu.Status.Success: raise newException(extras.AdapterError, "Failed to get the Adapter's information: " & $status)
